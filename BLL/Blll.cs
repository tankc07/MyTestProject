...
namespace BLL
{
    public class Blll
    {
        ...
        //Modify: 修改时间: 2024-02-22 By:Ly 修改内容:新增NewEms接口初始化.
        NewEms _newEms = NewEms.Init(APITokenKey.NewEmsSenderNo, APITokenKey.NewEmsSignKey, APITokenKey.NewEmsAuthorization, APITokenKey.NewEmsTestSignKey, APITokenKey.NewEmsTestAuthorization, APITokenKey.NewEmsBaseUrl, APITokenKey.NewEmsUrl, APITokenKey.NewEmsTestUrl);
        ...
        private Blll()
        {
            ...
            //Modify: 修改时间: 2024-02-22 By:Ly 修改内容:初始化NewEms接口测试标志为false.
            _newEms.IsTest = Settings.APITokenKey.NewEmsIsTest;
        }
...
";
        ...
        public void ServerStopLogic(BllMod.Order item, out bool isOk, out int errCode, out string errMsg)
        {
            ...
            //Modify: 修改时间: 2024-02-29 By:Ly 修改内容: 停止物流ServerStopLogic方法,新增NewEms接口分支
            else if (item.Logic == Setings.EnumLogicType.新邮政Ems)
            {
                DateTime jijianriqi;
                if (!DateTime.TryParse(item.logi_jinjianRiqi, out jijianriqi))
                {
                ...
                NewEmsRequestBase mod = _newEms.GetCancelOrderModel(
                    item.ErpId + (item.logi_SubOrderSn > 0 ? "_" + item.logi_SubOrderSn.ToString() : ""),
                    item.WL_NUMBER, jijianriqi, out isOk, out errCode, out errMsg);
                NewEmsResponseBase res = _newEms.SendNewEmsOrder(mod, out isOk, out errCode, out errMsg, out sJson, out rJson);
                if (isOk == true)
                {
                    var res2 = res as CancelOrderResponse;
                    if (res2 != null)
                    {
                        item.WL_NUMBER = "已删" + item.WL_NUMBER;
                        item.Status = Setings.EnumOrderStatus.停止完成;
...
where
	','||nvl(aa.orders_set,'')||',' like '%,{outid.ToString()},%' ";
            ...
            if (order != null)
            {
                if (YJT.Text.Verification.IsNotNullOrEmpty(order.ADDRESS))
                {
                    ...
                    if (logicStatus == true)
                    {
                        ...
                        {
                            #region 计算物流分配
                            order.Logic = SelectLogisticsProvider(order);
                            #endregion
                        }
                        ///设置发货人
                        ...
                        switch (order.Logic)
                        {
                            ...
                            //Modify: 修改时间: 2024-02-29 By:Ly 修改内容: 新增创建物流订单ServerCreateLogic2方法的[新邮政Ems]物流分支 
                            case Settings.Setings.EnumLogicType.新邮政Ems:
                                {
                                    ...
                                    order.needBaojia = 0d;
                                    CargoModel 药品info = _newEms.GetCargoModel(
                                        cargoCategory: "",
                                        cargoName: "药品",
                                        cargoQuantity: 0,
                                        cargoValue: 1,
                                        cargoLength: 0, cargoWidth: 0, cargoHigh: 0,
                                        cargoWeight: 0,
                                        orderNo: "",
                                        cargoMailNo: "");
                                    AddressModel 发件方 = _newEms.GetAddressModel(
                                        address: order.logi_sendAddress,
                                        prov: order.logi_sendSheng,
                                        city: order.logi_sendShi,
                                        county: order.logi_sendXian,
                                        ...
                                        phone: order.logi_sendPhone,
                                        postCode: "");
                                    AddressModel 收件方 = _newEms.GetAddressModel(
                                        address: order.ADDRESS,
                                        prov: order.PROVINCENAME,
                                        city: order.CITYNAME,
                                        county: order.DISTRICTNAME,
                                    ...
                                    order.logi_ReceivePwd = YJT.Text.ClassCreateText.FunStrCreateNumberStr(6, null);
                                    var diffWeightMsg = "";
                                    if ((int)order.Weight != order.Weight)
                                    {
                                    ...
                                    NewEmsRequestBase 取号 = _newEms.GetCreateOrderModel(
                                        ...
                                        erpId: order.ERPORDER_ID,
                                        length: order.Length, width: order.Width, height: order.Height, weight: (int)order.Weight,
                                        sender: 发件方,
                                        receiver: 收件方,
                                        ...
                                        pickupNotes: 备注,
                                        postageTotal: 0,
                                        receiverSafetyCode: order.logi_ReceivePwd);
                                    string stxt = "";
                                    string otxt = "";
                                    NewEmsResponseBase resobj = null;
                                    int tryCountEms = 5;
                                    while (tryCountEms > 0)
                                    {
                                        tryCountEms--;
                                        resobj = _newEms.SendNewEmsOrder(取号, out isOk, out errCode, out errMsg, out stxt, out otxt);
                                        try
                                        {
                                            if (!System.IO.Directory.Exists(@"D:\WLLOG"))
                                            {
                                    ...
                                    if (resobj != null)
                                    {
                                        if (isOk == true)
                                        {
                                            CreateOrderResponse 下单结果 = resobj as CreateOrderResponse;
                                            if (下单结果 != null)
                                            {
                                                //Modify: 修改时间: 2024-03-01 By:Ly 修改内容: 创建订单时,回写WL_COMPANYID和WL_COMPANYNAME时,为了回写电商平台, 还写原来的EMS相关信息.
                                                order.WL_COMPANYID = "2";
                                            ...
                                            else
                                            {
                                                ...
                                                isOk = false;
                                                errCode = -9;
                                                order.ErrMsg = "邮政物流新接口下单不成功:返回的对象不是CreateOrderResponse";
                                                errMsg = order.ErrMsg;
                                                order.Status = Settings.Setings.EnumOrderStatus.异常_物流下单不成功;
                                            }
                                        }
                                        else
                                        {
                                            ...
                                            isOk = false;
                                            errCode = -8;
                                            order.ErrMsg = "邮政物流新接口下单不成功:" + errMsg;
                                            errMsg = order.ErrMsg;
                                            order.Status = Settings.Setings.EnumOrderStatus.异常_物流下单不成功;
                                        }
                                    }
        ...
        /// <returns></returns>
        private static Setings.EnumLogicType SelectLogisticsProvider(BllMod.Order order)
        {
            #region 临时方案
            //// 申通3公斤及以内目的省列表
            //var shentongList = new List<string> { "辽宁", "江西", "浙江", "贵州", "重庆", "云南", "内蒙古", "甘肃", "陕西", "宁夏", "海南", "青海", "上海", "吉林", "福建", "广西" };

            //// 邮政3公斤及以内目的省列表
            //var emsList = new List<string> { "河北", "河南", "山东", "山西", "北京", "天津", "安徽", "湖北", "广东", "江苏", "湖南", "黑龙江", "四川" };

            //AddMsgOut("2 开始比配", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight, order.total_amt, true);
            ////1 订单金额大于2000元以上 >> 京东生鲜医药快递
            //if (Convert.ToDouble(order.total_amt) > 2000d)
            //{
            //    AddMsgOut("2.1 金额大于2000 >> 京东生鲜医药快递", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.total_amt);
            //    return Settings.Setings.EnumLogicType.京东生鲜医药快递;
            //}
            ////2 京津冀且重量大于3公斤 >> 京东生鲜医药快递
            //if (YJT.Text.Verification.IsLikeIn(order.PROVINCENAME, new List<string>() { "北京", "天津", "河北" }, true) && order.Weight > 3.0)
            //{
            //    AddMsgOut("2.2 属于京津冀且重量大于3.0 >> 京东生鲜医药快递", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);
            //    return Settings.Setings.EnumLogicType.京东生鲜医药快递;
            //}
            ////3 重量大于3公斤小于18公斤 >> 京东生鲜医药快递
            //if (order.Weight > 3.0 && order.Weight < 18.0)
            //{
            //    AddMsgOut("2.3 重量大于3.0小于20.0 >> 京东生鲜医药快递", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);
            //    return Setings.EnumLogicType.京东生鲜医药快递;
            //}
            ////4 重量小于等于3公斤 ->地区属于("河北", "河南", "山东", "山西", "北京", "天津", "安徽", "湖北", "广东", "江苏", "湖南", "黑龙江", "四川") >> 新邮政Ems
            ////                  ->地区属于("辽宁", "江西", "浙江", "贵州", "重庆", "云南", "内蒙古", "甘肃", "陕西", "宁夏", "海南", "青海", "上海", "吉林", "福建", "广西") >> 申通快递  
            //if (order.Weight <= 3.0)
            //{
            //    AddMsgOut("2.4 重量小于等于3.0 >> 需要进一步判断目的省", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);

            //    if (YJT.Text.Verification.IsLikeIn(order.PROVINCENAME, emsList, true))
            //    {
            //        AddMsgOut("2.4.1 属于[河北/河南/山东/山西/山东/北京/天津/安徽/湖北/广东/江苏/湖南/黑龙江/四川] >> 新邮政Ems", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);
            ...
            //    {
            //        AddMsgOut("2.4.2 属于[辽宁/江西/浙江/贵州/重庆/云南/内蒙古/甘肃/陕西/宁夏/海南/青海/上海/吉林/福建/广西] >> 申通", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);
            //        return Settings.Setings.EnumLogicType.申通快递;
            //    }
            //}
            ////5 以上都不符合的 默认 >> 京东生鲜医药快递
            //AddMsgOut("3 所有条件都不符合 >> 京东生鲜医药快递", Settings.Setings.EnumMessageType.提示, 0, "计算物流分配", order.ADDRESS, order.PROVINCENAME, order.Weight);
            //return Setings.EnumLogicType.京东生鲜医药快递;
            #endregion
...
";
        ...
        private BllMod.Order ServerCreateLogicSub(BllMod.Order orderNew, out bool isOk, out int errCode, out string errMsg)
        {
            ...
            if (orderNew != null)
            {
                ...
                //Modify: 修改时间:2024-02-29 By:Ly 修改内容: 创建子单ServerCreateLogicSub方法增加[新邮政Ems]分支, 并新建创建子单物流方法ServerCreateLogicSubNewEms()
                else if (orderNew.Logic == Settings.Setings.EnumLogicType.新邮政Ems)
                {
                    orderNew = ServerCreateLogicSubNewEms(orderNew, out isOk, out errCode, out errMsg);
                }
        ...
        //Modify: 修改时间: 2024-02-29 By:Ly 修改内容: 新增创建NewEms邮政新接口子单物流方法
        private BllMod.Order ServerCreateLogicSubNewEms(BllMod.Order order, out bool isOk, out int errCode, out string errMsg)
        {
            string 备注 = "";
            isOk = false;
            errCode = -1;
            ...
            if (order2 != null)
            {
                if (YJT.Text.Verification.IsNotNullOrEmpty(order2.WMS销售单号))
                {
                    ...
                    if (long.TryParse(order2.WMS销售单号, out wmsDanjbh))
                    {
                        ...
                        string sqlCmd = $@"update wms_out_order aa set aa.printmanid=2446,aa.printflag={printCount.ToString()},printdate={YJT.DataBase.Common.ConvertStringValue(
                        ...
                        if (logicStatus == true)
                        {
                            ...
                            switch (order.Logic)
                            {
                                ...
                                case Setings.EnumLogicType.邮政EMS:
                                    ...
                                    break;
                                //Modify: 修改时间: 2024-02-29 By:Ly 修改内容: ServerCreateLogic方法, 新增[新邮政Ems]分支
                                case Settings.Setings.EnumLogicType.新邮政Ems:
                                    {
                                        string 备注 = "";
                                        if (YJT.Text.Verification.IsNotNullOrEmpty(order.IsFp))
                ...
                if (order.Status == Settings.Setings.EnumOrderStatus.已获取物流单号)
                {
                    switch (order.Logic)
                    {
                        ...
                        case Settings.Setings.EnumLogicType.邮政EMS:
                            ...
                            flag = true;
                            break;
                        //Modify: 修改时间: 2024-02-29 By:Ly 修改内容: ServerLogicInfo方法新增[新邮政Ems]分支
                        case Settings.Setings.EnumLogicType.新邮政Ems:
                            order.ErrMsg = "物流信息获取完成";
                            flag = true;
                            break;
